...
describe("AIJudge commit-reveal (Obscura)", () => {
  let judge: any;
  ...
  beforeEach(async () => {
    ...
    await judge.write.createBounty(
      ["Best gas tip", "Clarity and correctness", subDeadline, revDeadline],
      { value: parseEther("1") },
    );
    bountyId = 1n;
  });
  ...
  it("hides the answer during the commit phase", async () => {
    const c = commitmentOf("pack structs", SALT_A, alice.account.address, bountyId);
  ...
  it("reveals a valid answer and stores it", async () => {
    const answer = "pack structs into one slot";
  ...
  it("getSubmissionIndex reports commitment presence on-chain", async () => {
    const c = commitmentOf("indexed", SALT_A, alice.account.address, bountyId);
    await judge.write.submitCommitment([bountyId, c], { account: alice.account });
