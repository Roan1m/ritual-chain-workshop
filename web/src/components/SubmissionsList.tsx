      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          {isWinner ? (
            ...
            <Badge tone="violet">AI pick</Badge>
          ) : null}
        </div>
      </div>

      {revealed ? (
        <p className="mt-2 whitespace-pre-wrap break-words text-sm text-stone-200">
          {answer ?? (isLoading ? "" : "-")}
        </p>
      ) : (
        <p className="mt-2 break-all font-mono text-xs text-stone-500">
          <span className="text-stone-600">commitment </span>
          {commitment ?? (isLoading ? "loading…" : "-")}
        </p>
      )}

      {ranking?.reason ? (
        <p className="mt-2 border-t border-white/5 pt-2 text-xs text-stone-400">
